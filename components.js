...
class ViettarotComponents {
    ...
    getComponentCSS() {
        ...
        @media (max-width: 767.98px) {
            /* Widget Cộng đồng vẫn ở dưới cùng bên phải */
            .widget-right {
                right: 15px;
                bottom: 15px;
                gap: 10px;
            }
            /* Widget Facebook di chuyển lên trên widget Cộng đồng, vẫn ở bên phải */
            .widget-left {
                left: auto; /* Đảm bảo không còn thuộc tính left */
                right: 15px;
                bottom: 80px; /* Đặt phía trên widget kia (15px đáy + 55px chiều cao + 10px khoảng cách) */
                flex-direction: row; /* Thay đổi để text luôn ở bên trái icon, tạo sự đồng nhất */
                gap: 10px;
            }
            .widget-left .messenger-text-bubble {
                /* Đổi điểm gốc của hiệu ứng phóng to cho phù hợp khi text ở bên trái */
                transform-origin: right bottom;
            }
            .widget-left .messenger-close-btn {
                right: -5px; /* Di chuyển nút đóng sang bên phải của icon */
                left: auto;
            }
            .messenger-fab-icon {
                width: 55px;
    ...
    async initializeRealtimePresence(user) {
        ...
        // Tạo một kênh duy nhất để theo dõi tất cả người dùng online
        ...
        this.presenceChannel.subscribe(async (status) => {
            if (status === 'SUBSCRIBED') {
                // Khi đăng ký thành công, bắt đầu theo dõi trạng thái
                // Gửi thông tin của người dùng để admin có thể thấy
        ...
        const updateLastSeen = async () => {
            const { error } = await this.supabase
                ...
                .eq('id', user.id);
            if (error) {
                console.error('Lỗi khi cập nhật last_seen:', error);
            }
        };
    
        // Cập nhật ngay khi online và sau đó cập nhật định kỳ mỗi 60 giây
        await updateLastSeen();
        this.lastSeenInterval = setInterval(updateLastSeen, 60000);
    }
    
    async leavePresenceChannel() {
        // Hủy đăng ký khỏi kênh
    ...
    updateUI(user) {
        ...
        if (user) {
            ...
            // START: Bắt đầu theo dõi khi có user
            this.initializeRealtimePresence(user); 
            // END: Bắt đầu theo dõi khi có user
        } else {
            authContainer.innerHTML = `
            ...
            `;
             // START: Dừng theo dõi khi không có user
            this.leavePresenceChannel();
            // END: Dừng theo dõi khi không có user
        }
    ...
    async handleLogout(event) {
        event.preventDefault();
        // START: Ngắt kết nối trước khi logout
        await this.leavePresenceChannel(); 
        // END: Ngắt kết nối trước khi logout
