...
<head>
    ...
    <link rel="icon" type="image/png" href="/image/iconlogo.png">
    ...
    <style>
        ...
        .price-member {
            color: #4A6CF3;
            background-color: rgba(74, 108, 243, 0.1);
        }
        ...
        .filter-btn.filter-btn-student {
            background: linear-gradient(90deg, #ffc107, #ff9800);
            color: #212529;
            border-color: transparent;
            font-weight: 600;
        }
        .filter-btn.filter-btn-student:hover:not(.active) {
            box-shadow: 0 4px 15px rgba(255, 152, 0, 0.4);
            transform: translateY(-2px) scale(1.05);
            background: linear-gradient(90deg, #ffca2c, #ffa726);
        }
        .filter-btn.filter-btn-student.active {
             background: linear-gradient(90deg, #4A6CF3 0%, #9B49F2 100%);
             color: #FFFFFF;
             box-shadow: 0 4px 15px rgba(120, 73, 242, 0.3);
        }
        ...
        #documentDetailInfo .access-section {
            margin: 1.5rem 0;
        }
        ...
        #permissionDeniedModal .modal-header {
            border-bottom: none;
        }
...
<body>
                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            <div class="d-flex flex-wrap align-items-center gap-2 mb-4" id="document-filters">
                                ...
                                <button class="btn filter-btn" data-filter-type="member">Member</button>
                                <button class="btn filter-btn filter-btn-student" data-filter-type="student">Học viên</button>
    ...
    <script>
        ...
        function initializeDocumentPage() {
            ...
            // NEW: Khởi tạo modal thông báo quyền truy cập
            const permissionModalEl = document.getElementById('permissionDeniedModal');
            ...
            // NEW: Hàm hiển thị popup thông báo quyền
            function showPermissionModal(requiredRole) {
                if (permissionModalBody) {
                    permissionModalBody.innerHTML = `<p class="fs-5 mb-1">Bạn cần là <strong>${requiredRole}</strong> để xem nội dung này.</p><p class="text-secondary">Vui lòng đăng nhập hoặc nâng cấp tài khoản của bạn.</p>`;
                }
            ...
            function renderDocuments(docs) {
                ...
                docs.forEach(doc => {
                    ...
                    switch (doc.access_level) {
                        case 'free':
                            accessLevelHTML = `<span class="price-badge price-free">Miễn phí</span>`;
                            break;
                        case 'member':
                            accessLevelHTML = `<span class="price-badge price-member">Dành cho Member</span>`;
                            break;
                        case 'student':
                             accessLevelHTML = `<span class="price-badge price-student">Dành cho Học viên</span>`;
                            break;
                        default:
                             accessLevelHTML = `<span class="price-badge price-free">Công khai</span>`;
                    }
                        ...
                        <div class="col-lg-4 col-md-6 col-6 d-flex"> 
                            <div class="document-card" data-doc-id="${doc.id}">
                                ...
                                <div class="document-card-body">
                                    ...
                                    <div class="document-card-access">${accessLevelHTML}</div>
            ...
            function applyFilters() {
                ...
                const filterType = activeFilters.type;
                
                if (filterType === 'free') {
                    filtered = filtered.filter(doc => doc.access_level === 'free');
                } else if (filterType === 'member') {
                    filtered = filtered.filter(doc => doc.access_level === 'member');
                } else if (filterType === 'student') {
                    filtered = filtered.filter(doc => doc.access_level === 'student');
                } else if (filterType === 'all') {
                    // **CODE ĐÃ SỬA**
                    // Khi chọn "Tất cả", không lọc theo quyền truy cập ở đây nữa.
                    // Tất cả tài liệu sẽ được hiển thị.
                    // Việc kiểm tra quyền sẽ được thực hiện khi người dùng nhấn vào xem.
                }
            ...
            function populateModal(doc) {
                ...
                let accessLevelHTML = '';
                switch (doc.access_level) {
                    case 'free': accessLevelHTML = `<span class="price-badge price-free">Miễn phí</span>`; break;
                    case 'member': accessLevelHTML = `<span class="price-badge price-member">Dành cho Member</span>`; break;
                    case 'student': accessLevelHTML = `<span class="price-badge price-student">Dành cho Học viên</span>`; break;
                    default: accessLevelHTML = `<span class="price-badge price-free">Công khai</span>`;
                }
                ...
                detailModalBody.innerHTML = `
                    ...
                    <div id="documentDetailInfo">
                        ...
                        <div class="access-section">${accessLevelHTML}</div>
            ...
            async function fetchUserRole() {
                ...
                if (user) {
                    ...
                    if (profile) userRole = profile.role; else userRole = 'user';
                } else {
                    userRole = 'guest';
                }
            }
            ...
            async function loadInitialData() {
                ...
                if (!popularRes.error && popularRes.data) renderSidebarDocs(popularDocumentsContainer, popularRes.data.slice(0, 4));
            }

            filtersContainer?.addEventListener('click', (e) => {
                const targetBtn = e.target.closest('.filter-btn');
                const targetDropdown = e.target.closest('.dropdown-item');

                if (targetBtn) {
                    filtersContainer.querySelector('.filter-btn.active')?.classList.remove('active');
                    targetBtn.classList.add('active');
                    activeFilters.type = targetBtn.dataset.filterType;
                    applyFilters();
                }

                if (targetDropdown) {
                    const category = targetDropdown.dataset.category;
                    activeFilters.category = category;
                    if(categoryFilterDropdown) categoryFilterDropdown.textContent = category === 'all' ? 'Tất cả danh mục' : targetDropdown.textContent;
                    applyFilters();
                }
            });
            ...
            document.body.addEventListener('click', (e) => {
                 const trigger = e.target.closest('.document-card, .widget-post-thumbnail, .widget-post-content h6 a');
                 if(trigger && trigger.dataset.docId) {
                     const docId = trigger.dataset.docId;
                     const doc = allDocuments.find(d => d.id == docId);
                     
                     if (doc) {
                        const requiredLevel = doc.access_level;
                        let hasPermission = false;

                        // Kiểm tra quyền truy cập của người dùng
                        if (requiredLevel === 'free') {
                            hasPermission = true;
                        } else if (requiredLevel === 'member' && (userRole === 'membership' || userRole === 'student')) {
                            hasPermission = true;
                        } else if (requiredLevel === 'student' && userRole === 'student') {
                            hasPermission = true;
                        }
                        
                        // Nếu có quyền, hiển thị chi tiết tài liệu
                        if(hasPermission) {
                            ...
                            // Nếu không có quyền, hiển thị thông báo
                             let requiredRoleText = 'Member'; // Mặc định
                             if (requiredLevel === 'student') {
                                 requiredRoleText = 'Học viên';
                             }
                             showPermissionModal(requiredRoleText);
                        }
                     }
                 }
            });
